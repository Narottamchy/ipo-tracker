...
export default function AiAnalysis({ slug, id }) {
  ...
  return (
    <section className="rounded-2xl border border-ink-700 bg-ink-900 p-5 shadow-card sm:p-6">
      ...
      {state === 'idle' && (
        <div className="mt-4">
          <p className="text-xs leading-relaxed text-ink-400">
            Runs a scoring rubric against this IPO&apos;s live GMP and subscription data. Shared across all visitors —
            computed once, then cached for everyone.
          </p>
          <button
            type="button"
            onClick={run}
      ...
      {state === 'done' && result && (
        <div className="mt-4">
          <p className="text-[10px] text-ink-400">Generated {formatDateTime(result.generatedAt)} · shared cache</p>

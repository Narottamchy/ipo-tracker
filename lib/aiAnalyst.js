...
import { kvDel, kvGet, kvLock, kvSet } from './kv.js';
...
function memSet(key, value) {
  ...
  cache.set(key, { value, expiresAt: Date.now() + MEMORY_MS });
}
...
// Read-only lookup for page renders — never calls NVIDIA/Tavily, just checks what's already been
// generated (memory, then Redis).
...
// Generated once per IPO and shared with everyone until the IPO lists (Redis expiry). Only one
// instance generates at a time (Redis lock); others wait for its result. `replace` is reserved for
// the scheduled job, which refreshes a pre-close analysis once with final subscription numbers.
export async function getAiAnalysis(slug, id, detailData, { replace = false } = {}) {
  const key = `${slug}/${id}`;
  if (!replace) {
  ...
  const promise = (async () => {
    const lockKey = `lock:ai:${key}`;
    if (!(await kvLock(lockKey, 150))) {
      for (let waited = 0; waited < 75_000; waited += 3000) {
        await sleep(3000);
        const ready = await kvGet(`ai-analysis:${key}`);
        if (ready && (!replace || ready.stage === 'closed')) return ready;
      }
      throw Object.assign(new Error('This analysis is being prepared. Please check back in a minute.'), { status: 503 });
    }
    try {
      ...
      const result = {
        analysis,
        model: MODEL,
        stage: detailData.statusCode === 'C' ? 'closed' : 'pre',
...
// Scheduled-job entry point: analysis for a closed IPO should reflect final subscription numbers,
// so replace a pre-close analysis exactly once. Returns true if it generated something.
export async function ensureClosedAnalysis(slug, id, detailData) {
  const cached = await getCachedAnalysis(slug, id);
  if (cached?.stage === 'closed') return false;
  await getAiAnalysis(slug, id, detailData, { replace: true });
  return true;
}

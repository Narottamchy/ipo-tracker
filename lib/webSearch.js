...
export async function researchIpo(name, isSme, issueData = null) {
  const legal = `${name} IPO promoter director SEBI case litigation red flags India`;
  const queries = issueData
    ? [
        legal,
        isSme
          ? `${issueData.leadManagers?.[0] || name} lead manager SME IPO track record listing performance India`
          : `${name} IPO peer companies valuation P/E comparison India`,
      ]
    : [
        legal,
